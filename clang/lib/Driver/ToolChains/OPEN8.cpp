//===--- OPEN8.cpp - OPEN8 ToolChain Implementations ----------------*- C++ -*-===//
//
...
const struct {
  ...
  unsigned DataAddr;
    ...
    {"open8generic_0", "", "open81", 0},
    {"open8generic_8", "", "open81", 0x8000},
};
...
const StringRef PossibleOPEN8LibcLocations[] = {
    "/usr/open8",
    "/usr/lib/open8",
};
...
/// OPEN8 Toolchain
OPEN8ToolChain::OPEN8ToolChain(const Driver &D, const llvm::Triple &Triple,
                           const ArgList &Args)
  ...
  if (!Args.hasArg(options::OPT_nostdlib) &&
    ...
    if (CPU.empty()) {
      ...
      D.Diag(diag::warn_drv_open8_mcu_not_specified);
    } else {
      Optional<StringRef> FamilyName = GetMCUFamilyName(CPU);
      Optional<std::string> OPEN8LibcRoot = findOPEN8LibcInstallation();

      if (!FamilyName.hasValue()) {
        // We do not have an entry for this CPU in the family
        // mapping table yet.
        D.Diag(diag::warn_drv_open8_family_linking_stdlibs_not_implemented)
            << CPU;
      } else if (!GCCInstallation.isValid()) {
        // No open8-gcc found and so no runtime linked.
        D.Diag(diag::warn_drv_open8_gcc_not_found);
      } else if (!OPEN8LibcRoot.hasValue()) {
        // No open8-libc found and so no runtime linked.
        D.Diag(diag::warn_drv_open8_libc_not_found);
        ...
        std::string LibcRoot = OPEN8LibcRoot.getValue();
    ...
    if (!LinkStdlib)
      D.Diag(diag::warn_drv_open8_stdlib_not_linked);
  }
}

Tool *OPEN8ToolChain::buildLinker() const {
  return new tools::OPEN8::Linker(getTriple(), *this, LinkStdlib);
}

void OPEN8::Linker::ConstructJob(Compilation &C, const JobAction &JA,
                               const InputInfo &Output,
                               const InputInfoList &Inputs, const ArgList &Args,
                               const char *LinkingOutput) const {
  // Compute information about the target OPEN8.
  ...
  if (SectionAddressData.hasValue()) {
    ...
    getToolChain().getDriver().Diag(
        diag::warn_drv_open8_linker_section_addresses_not_implemented)
        << CPU;
  }

  // If the family name is known, we can link with the device-specific libgcc.
  // Without it, libgcc will simply not be linked. This matches open8-gcc
  // behavior.
  if (LinkStdlib) {
    ...
    // This is almost always required because otherwise open8-ld
    // will assume 'open82' and warn about the program being larger
...
llvm::Optional<std::string> OPEN8ToolChain::findOPEN8LibcInstallation() const {
  for (StringRef PossiblePath : PossibleOPEN8LibcLocations) {
    std::string Path = getDriver().SysRoot + PossiblePath.str();
    // Return the first open8-libc installation that exists.
