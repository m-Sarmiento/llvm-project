//===-- OPEN8MCCodeEmitter.cpp - Convert OPEN8 Code to Machine Code -----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the OPEN8MCCodeEmitter class.
//
//===----------------------------------------------------------------------===//

#include "OPEN8MCCodeEmitter.h"
...
unsigned
OPEN8MCCodeEmitter::loadStorePostEncoder(const MCInst &MI, unsigned EncodedValue,
...
template <OPEN8::Fixups Fixup>
unsigned
OPEN8MCCodeEmitter::encodeRelCondBrTarget(const MCInst &MI, unsigned OpNo,
...
unsigned OPEN8MCCodeEmitter::encodeLDSTPtrReg(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
...
unsigned OPEN8MCCodeEmitter::encodeMemri(const MCInst &MI, unsigned OpNo,
                                       SmallVectorImpl<MCFixup> &Fixups,
                                       const MCSubtargetInfo &STI) const {
  ...
  if (OffsetOp.isImm()) {
    ...
    Fixups.push_back(MCFixup::create(0, OffsetOp.getExpr(),
                     MCFixupKind(OPEN8::fixup_6), MI.getLoc()));
  } else {
...
unsigned OPEN8MCCodeEmitter::encodeComplement(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
...
template <OPEN8::Fixups Fixup, unsigned Offset>
unsigned OPEN8MCCodeEmitter::encodeImm(const MCInst &MI, unsigned OpNo,
                                     SmallVectorImpl<MCFixup> &Fixups,
                                     const MCSubtargetInfo &STI) const {
  auto MO = MI.getOperand(OpNo);

  if (MO.isExpr()) {
    if (isa<OPEN8MCExpr>(MO.getExpr())) {
      // If the expression is already an OPEN8MCExpr (i.e. a lo8(symbol),
...
unsigned OPEN8MCCodeEmitter::encodeCallTarget(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  auto MO = MI.getOperand(OpNo);

  if (MO.isExpr()) {
    MCFixupKind FixupKind = static_cast<MCFixupKind>(OPEN8::fixup_call);
...
unsigned OPEN8MCCodeEmitter::getExprOpValue(const MCExpr *Expr,
                                          SmallVectorImpl<MCFixup> &Fixups,
                                          const MCSubtargetInfo &STI) const {
  ...
  if (Kind == MCExpr::Target) {
    OPEN8MCExpr const *OPEN8Expr = cast<OPEN8MCExpr>(Expr);
    int64_t Result;
    if (OPEN8Expr->evaluateAsConstant(Result)) {
      return Result;
    }

    MCFixupKind FixupKind = static_cast<MCFixupKind>(OPEN8Expr->getFixupKind());
    Fixups.push_back(MCFixup::create(0, OPEN8Expr, FixupKind));
    return 0;
  }
...
unsigned OPEN8MCCodeEmitter::getMachineOpValue(const MCInst &MI,
                                             const MCOperand &MO,
                                             SmallVectorImpl<MCFixup> &Fixups,
                                             const MCSubtargetInfo &STI) const {
...
MCCodeEmitter *createOPEN8MCCodeEmitter(const MCInstrInfo &MCII,
                                      ...
                                      MCContext &Ctx) {
  return new OPEN8MCCodeEmitter(MCII, Ctx);
}

#include "OPEN8GenMCCodeEmitter.inc"
