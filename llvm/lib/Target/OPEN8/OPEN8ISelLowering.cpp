//===-- OPEN8ISelLowering.cpp - OPEN8 DAG Lowering Implementation -------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that OPEN8 uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
OPEN8TargetLowering::OPEN8TargetLowering(const OPEN8TargetMachine &TM,
                                     const OPEN8Subtarget &STI)
    : TargetLowering(TM), Subtarget(STI) {
  // Set up the register classes.
  addRegisterClass(MVT::i8, &OPEN8::GPR8RegClass);
  addRegisterClass(MVT::i16, &OPEN8::DREGSRegClass);
  ...
  setLibcallCallingConv(RTLIB::SDIVREM_I16, CallingConv::OPEN8_BUILTIN);
  setLibcallCallingConv(RTLIB::UDIVREM_I8, CallingConv::OPEN8_BUILTIN);
  setLibcallCallingConv(RTLIB::UDIVREM_I16, CallingConv::OPEN8_BUILTIN);
...
const char *OPEN8TargetLowering::getTargetNodeName(unsigned Opcode) const {
#define NODE(name)       \
  case OPEN8ISD::name:     \
    return #name
...
EVT OPEN8TargetLowering::getSetCCResultType(const DataLayout &DL, LLVMContext &,
                                          EVT VT) const {
  assert(!VT.isVector() && "No OPEN8 SetCC type for vectors!");
  return MVT::i8;
}

SDValue OPEN8TargetLowering::LowerShifts(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (!isa<ConstantSDNode>(N->getOperand(1))) {
    ...
    case ISD::SHL:
      return DAG.getNode(OPEN8ISD::LSLLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::SRL:
      return DAG.getNode(OPEN8ISD::LSRLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::ROTL: {
      ...
      return DAG.getNode(OPEN8ISD::ROLLOOP, dl, VT, N->getOperand(0), Amt);
    }
    case ISD::ROTR: {
      SDValue Amt = N->getOperand(1);
      EVT AmtVT = Amt.getValueType();
      Amt = DAG.getNode(ISD::AND, dl, AmtVT, Amt,
                        DAG.getConstant(VT.getSizeInBits() - 1, dl, AmtVT));
      return DAG.getNode(OPEN8ISD::RORLOOP, dl, VT, N->getOperand(0), Amt);
    }
    case ISD::SRA:
      return DAG.getNode(OPEN8ISD::ASRLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    }
  }
  ...
  if (VT.getSizeInBits() == 8) {
    /*if (Op.getOpcode() == ISD::SHL && 4 <= ShiftAmount && ShiftAmount < 7) {
      ...
      Victim = DAG.getNode(OPEN8ISD::SWAP, dl, VT, Victim);
      Victim =
    ...
    /*if (4 <= ShiftAmount && ShiftAmount < 8)
      switch (Op.getOpcode()) {
      case ISD::SHL:
        Victim = DAG.getNode(OPEN8ISD::LSL4, dl, VT, Victim);
        ShiftAmount -= 4;
        break;
      case ISD::SRL:
        Victim = DAG.getNode(OPEN8ISD::LSR4, dl, VT, Victim);
        ShiftAmount -= 4;
        break;
      default:
        break;
      ...
      switch (Op.getOpcode()) {
      case ISD::SHL:
        Victim = DAG.getNode(OPEN8ISD::LSL8, dl, VT, Victim);
        ShiftAmount -= 8;
        break;
      case ISD::SRL:
        Victim = DAG.getNode(OPEN8ISD::LSR8, dl, VT, Victim);
        ShiftAmount -= 8;
        break;
      case ISD::SRA:
        Victim = DAG.getNode(OPEN8ISD::ASR8, dl, VT, Victim);
        ShiftAmount -= 8;
        break;
      default:
        break;
    ...
    /*else if (12 <= ShiftAmount)
      switch (Op.getOpcode()) {
      case ISD::SHL:
        Victim = DAG.getNode(OPEN8ISD::LSL12, dl, VT, Victim);
        ShiftAmount -= 12;
        break;
      case ISD::SRL:
        Victim = DAG.getNode(OPEN8ISD::LSR12, dl, VT, Victim);
        ShiftAmount -= 12;
        break;
      default:
        break;
...
SDValue OPEN8TargetLowering::LowerDivRem(SDValue Op, SelectionDAG &DAG) const {
...
SDValue OPEN8TargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(OPEN8ISD::WRAPPER, SDLoc(Op), getPointerTy(DL), Result);
}

SDValue OPEN8TargetLowering::LowerBlockAddress(SDValue Op,
                                             SelectionDAG &DAG) const {
  auto DL = DAG.getDataLayout();
...
/// IntCCToOPEN8CC - Convert a DAG integer condition code to an OPEN8 CC.
static OPEN8CC::CondCodes intCCToOPEN8CC(ISD::CondCode CC) {
  switch (CC) {
  default:
...
SDValue OPEN8TargetLowering::getOPEN8Cmp(SDValue LHS, SDValue RHS,
  ...
  if (LHS.getSimpleValueType() == MVT::i16 && isa<ConstantSDNode>(RHS)) {
    ...
    SDValue RHShi = DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8, RHS,
                                DAG.getIntPtrConstant(1, DL));
    Cmp = DAG.getNode(OPEN8ISD::CMP, DL, MVT::Glue, LHSlo, RHSlo);
    Cmp = DAG.getNode(OPEN8ISD::CMPC, DL, MVT::Glue, LHShi, RHShi, Cmp);
  } else {
    // Generate ordinary 16-bit comparison.
    Cmp = DAG.getNode(OPEN8ISD::CMP, DL, MVT::Glue, LHS, RHS);
  }

  return Cmp;
}

/// Returns appropriate OPEN8 CMP/CMPC nodes and corresponding condition code for
/// the given operands.
SDValue OPEN8TargetLowering::getOPEN8Cmp(SDValue LHS, SDValue RHS, ISD::CondCode CC,
                                     SDValue &OPEN8cc, SelectionDAG &DAG,
                                     SDLoc DL) const {
  SDValue Cmp;
  ...
  case ISD::SETGT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case -1: {
        ...
        UseTest = true;
        OPEN8cc = DAG.getConstant(OPEN8CC::COND_PL, DL, MVT::i8);
        break;
      }
      case 0: {
  ...
  case ISD::SETLT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case 0: {
        ...
        UseTest = true;
        OPEN8cc = DAG.getConstant(OPEN8CC::COND_MI, DL, MVT::i8);
        break;
      }
      }
    }
  ...
  if (VT == MVT::i32) {
    ...
    if (UseTest) {
      ...
      SDValue Top = DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8, LHShi,
                                DAG.getIntPtrConstant(1, DL));
      Cmp = DAG.getNode(OPEN8ISD::TST, DL, MVT::Glue, Top);
    } else {
      Cmp = getOPEN8Cmp(LHSlo, RHSlo, DAG, DL);
      Cmp = DAG.getNode(OPEN8ISD::CMPC, DL, MVT::Glue, LHShi, RHShi, Cmp);
    }
  } else if (VT == MVT::i64) {
    ...
    if (UseTest) {
      ...
      Cmp = getOPEN8Cmp(LHS0, RHS0, DAG, DL);
      Cmp = DAG.getNode(OPEN8ISD::CMPC, DL, MVT::Glue, LHS1, RHS1, Cmp);
      Cmp = DAG.getNode(OPEN8ISD::CMPC, DL, MVT::Glue, LHS2, RHS2, Cmp);
      Cmp = DAG.getNode(OPEN8ISD::CMPC, DL, MVT::Glue, LHS3, RHS3, Cmp);
    }
  } else if (VT == MVT::i8 || VT == MVT::i16) {
    if (UseTest) {
      // When using tst we only care about the highest part.
      Cmp = DAG.getNode(OPEN8ISD::TST, DL, MVT::Glue,
                        (VT == MVT::i8)
                            ? LHS
                            : DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8,
                                          LHS, DAG.getIntPtrConstant(1, DL)));
    } else {
      Cmp = getOPEN8Cmp(LHS, RHS, DAG, DL);
    }
  } else {
    llvm_unreachable("Invalid comparison size");
  }

  // When using a test instruction OPEN8cc is already set.
  if (!UseTest) {
    OPEN8cc = DAG.getConstant(intCCToOPEN8CC(CC), DL, MVT::i8);
  }

  return Cmp;
}

SDValue OPEN8TargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue TargetCC;
  SDValue Cmp = getOPEN8Cmp(LHS, RHS, CC, TargetCC, DAG, dl);

  return DAG.getNode(OPEN8ISD::BRCOND, dl, MVT::Other, Chain, Dest, TargetCC,
                     Cmp);
}

SDValue OPEN8TargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(OPEN8ISD::SELECT_CC, dl, VTs, Ops);
}

SDValue OPEN8TargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
...
SDValue OPEN8TargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
...
SDValue OPEN8TargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
...
void OPEN8TargetLowering::ReplaceNodeResults(SDNode *N,
...
bool OPEN8TargetLowering::isLegalAddressingMode(const DataLayout &DL,
  ...
  if (isa<PointerType>(Ty) && AS == OPEN8::ProgramMemory) {
    return false;
  }
...
bool OPEN8TargetLowering::getPreIndexedAddressParts(SDNode *N, SDValue &Base,
                                                  SDValue &Offset,
  ...
  if (const LoadSDNode *LD = dyn_cast<LoadSDNode>(N)) {
    ...
    if (LD->getExtensionType() != ISD::NON_EXTLOAD)
      return false;
    if (OPEN8::isProgramMemoryAccess(LD)) {
      return false;
    }
  } else if (const StoreSDNode *ST = dyn_cast<StoreSDNode>(N)) {
    VT = ST->getMemoryVT();
    Op = ST->getBasePtr().getNode();
    if (OPEN8::isProgramMemoryAccess(ST)) {
      return false;
    }
  } else {
    return false;
...
bool OPEN8TargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                   SDValue &Base,
                                                   SDValue &Offset,
                                                   ISD::MemIndexedMode &AM,
                                                   SelectionDAG &DAG) const {
...
bool OPEN8TargetLowering::isOffsetFoldingLegal(
...
#include "OPEN8GenCallingConv.inc"
...
SDValue OPEN8TargetLowering::LowerFormalArguments(
  ...
  if (isVarArg) {
    CCInfo.AnalyzeFormalArguments(Ins, ArgCC_OPEN8_Vararg);
  } else {
  ...
  for (CCValAssign &VA : ArgLocs) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (RegVT == MVT::i8) {
        RC = &OPEN8::GPR8RegClass;
      } else if (RegVT == MVT::i16) {
        RC = &OPEN8::DREGSRegClass;
      } else {
...
SDValue OPEN8TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // OPEN8 does not yet support tail call optimization.
  isTailCall = false;
  ...
  // Variadic functions do not need all the analysis below.
  if (isVarArg) {
    CCInfo.AnalyzeCallOperands(Outs, ArgCC_OPEN8_Vararg);
  } else {
  ...
  Chain = DAG.getNode(OPEN8ISD::CALL, DL, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
///
SDValue OPEN8TargetLowering::LowerCallResult(
  ...
  if (CallConv == CallingConv::OPEN8_BUILTIN) {
    CCInfo.AnalyzeCallResult(Ins, RetCC_OPEN8_BUILTIN);
  } else {
...
bool OPEN8TargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool isVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context) const {
  if (CallConv == CallingConv::OPEN8_BUILTIN) {
    SmallVector<CCValAssign, 16> RVLocs;
    CCState CCInfo(CallConv, isVarArg, MF, RVLocs, Context);
    return CCInfo.CheckReturn(Outs, RetCC_OPEN8_BUILTIN);
  }
...
SDValue
OPEN8TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool isVarArg,
  ...
  if (CallConv == CallingConv::OPEN8_BUILTIN) {
    CCInfo.AnalyzeReturn(Outs, RetCC_OPEN8_BUILTIN);
  } else {
...
MachineBasicBlock *OPEN8TargetLowering::insertShift(MachineInstr &MI,
  ...
  default:
    llvm_unreachable("Invalid shift opcode!");
  case OPEN8::Lsl8:
    Opc = OPEN8::ADDRdRr; // LSL is an alias of ADD Rd, Rd
    RC = &OPEN8::GPR8RegClass;
  ...
  Register ShiftAmtReg = RI.createVirtualRegister(&OPEN8::GPR8RegClass);
  Register ShiftAmtReg2 = RI.createVirtualRegister(&OPEN8::GPR8RegClass);
  ...
  BuildMI(CheckBB, dl, TII.get(OPEN8::PHI), ShiftReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);
  BuildMI(CheckBB, dl, TII.get(OPEN8::PHI), ShiftAmtReg)
      .addReg(ShiftAmtSrcReg)
      .addMBB(BB)
      .addReg(ShiftAmtReg2)
      .addMBB(LoopBB);
  BuildMI(CheckBB, dl, TII.get(OPEN8::PHI), DstReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);
...
static bool isCopyMulResult(MachineBasicBlock::iterator const &I) {
  if (I->getOpcode() == OPEN8::COPY) {
    Register SrcReg = I->getOperand(1).getReg();
    return (SrcReg == OPEN8::R0 || SrcReg == OPEN8::R1);
  }
...
MachineBasicBlock *OPEN8TargetLowering::insertMul(MachineInstr &MI,
                                                MachineBasicBlock *BB) const {
  const TargetInstrInfo &TII = *Subtarget.getInstrInfo();
  ...
  if (isCopyMulResult(I))
    ++I;
  BuildMI(*BB, I, MI.getDebugLoc(), TII.get(OPEN8::EORRdRr), OPEN8::R1)
      .addReg(OPEN8::R1)
      .addReg(OPEN8::R1);
  return BB;
}
...
MachineBasicBlock *
OPEN8TargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  assert((Opc == OPEN8::Select16 || Opc == OPEN8::Select8) &&
         "Unexpected instr type to insert");

  const OPEN8InstrInfo &TII = (const OPEN8InstrInfo &)*MI.getParent()
                                ->getParent()
                                ->getSubtarget()
                                .getInstrInfo();
  DebugLoc dl = MI.getDebugLoc();
  ...
  BuildMI(*trueMBB, trueMBB->begin(), dl, TII.get(OPEN8::PHI), MI.getOperand(0).getReg())
...
OPEN8TargetLowering::ConstraintType
OPEN8TargetLowering::getConstraintType(StringRef Constraint) const {
  if (Constraint.size() == 1) {
    // See http://www.nongnu.org/open8-libc/user-manual/inline_asm.html
    switch (Constraint[0]) {
    default:
      break;
...
unsigned
OPEN8TargetLowering::getInlineAsmMemConstraint(StringRef ConstraintCode) const {
...
OPEN8TargetLowering::ConstraintWeight
OPEN8TargetLowering::getSingleConstraintMatchWeight(
...
OPEN8TargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    ...
    case 'a': // Simple upper registers r16..r23.
      return std::make_pair(0U, &OPEN8::LD8loRegClass);
    case 'b': // Base pointer registers: y, z.
      return std::make_pair(0U, &OPEN8::PTRDISPREGSRegClass);
    case 'd': // Upper registers r16..r31.
      return std::make_pair(0U, &OPEN8::LD8RegClass);
    case 'l': // Lower registers r0..r15.
      return std::make_pair(0U, &OPEN8::GPR8loRegClass);
    case 'e': // Pointer register pairs: x, y, z.
      return std::make_pair(0U, &OPEN8::PTRREGSRegClass);
    case 'q': // Stack pointer register: SPH:SPL.
      return std::make_pair(0U, &OPEN8::GPRSPRegClass);
    case 'r': // Any register: r0..r31.
      if (VT == MVT::i8)
        return std::make_pair(0U, &OPEN8::GPR8RegClass);

      return std::make_pair(0U, &OPEN8::DREGSRegClass);
...
void OPEN8TargetLowering::LowerAsmOperandForConstraint(SDValue Op,
...
Register OPEN8TargetLowering::getRegisterByName(const char *RegName, LLT VT,
