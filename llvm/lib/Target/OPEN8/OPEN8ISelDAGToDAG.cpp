...
template <> bool OPEN8DAGToDAGISel::select<OPEN8ISD::CALL>(SDNode *N) {
  ...
  ReplaceUses(SDValue(N, 1), SDValue(ResNode, 1));
  CurDAG->RemoveDeadNode(N);*/
  LLVM_DEBUG(errs() << "unsupported indirect CALL");
  return true;
}
...
template <> bool OPEN8DAGToDAGISel::select<ISD::BRIND>(SDNode *N) {
  ...
  SDNode *ResNode = CurDAG->getMachineNode(OPEN8::JMPk, DL, MVT::Other, Chain);

  ReplaceUses(SDValue(N, 0), SDValue(ResNode, 0));
  CurDAG->RemoveDeadNode(N);
*/
  LLVM_DEBUG(errs() << "unsupported indirect branch");
  return true;
}
