//===-- OPEN8ISelDAGToDAG.cpp - A dag to dag inst selector for OPEN8 ----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the OPEN8 target.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "open8-isel"

namespace llvm {

/// Lowers LLVM IR (in DAG form) to OPEN8 MC instructions (in DAG form).
class OPEN8DAGToDAGISel : public SelectionDAGISel {
public:
  OPEN8DAGToDAGISel(OPEN8TargetMachine &TM, CodeGenOpt::Level OptLevel)
      : SelectionDAGISel(TM, OptLevel), Subtarget(nullptr) {}

  StringRef getPassName() const override {
    return "OPEN8 DAG->DAG Instruction Selection";
  }
...
bool OPEN8DAGToDAGISel::runOnMachineFunction(MachineFunction &MF) {
  Subtarget = &MF.getSubtarget<OPEN8Subtarget>();
  return SelectionDAGISel::runOnMachineFunction(MF);
}

bool OPEN8DAGToDAGISel::SelectAddr(SDNode *Op, SDValue N, SDValue &Base,
                                 SDValue &Disp) {
  SDLoc dl(Op);
...
bool OPEN8DAGToDAGISel::selectIndexedLoad(SDNode *N) {
  ...
  case MVT::i8: {
    ...
    Opcode = (isPre) ? OPEN8::LDRdPtrPd : OPEN8::LDRdPtrPi;
    break;
  }
  case MVT::i16: {
    ...
    Opcode = (isPre) ? OPEN8::LDWRdPtrPd : OPEN8::LDWRdPtrPi;
    break;
  }
  default:
    return false;
...
unsigned OPEN8DAGToDAGISel::selectIndexedProgMemLoad(const LoadSDNode *LD,
                                                   MVT VT) {
  ISD::MemIndexedMode AM = LD->getAddressingMode();
...
bool OPEN8DAGToDAGISel::SelectInlineAsmMemoryOperand(const SDValue &Op,
  ...
  if (RegNode &&
      RI.getRegClass(RegNode->getReg()) == &OPEN8::PTRDISPREGSRegClass) {
    OutOps.push_back(Op);
    return false;
  }
  ...
  if (Op->getOpcode() == ISD::ADD || Op->getOpcode() == ISD::SUB) {
    ...
    if (CopyFromRegOp->getOpcode() == ISD::CopyFromReg) {
      ...
      CanHandleRegImmOpt &= (Register::isVirtualRegister(Reg) ||
                             OPEN8::PTRDISPREGSRegClass.contains(Reg));
    } else {
    ...
    if (CanHandleRegImmOpt) {
      ...
      if (RI.getRegClass(Reg) != &OPEN8::PTRDISPREGSRegClass) {
        SDLoc dl(CopyFromRegOp);

        Register VReg = RI.createVirtualRegister(&OPEN8::PTRDISPREGSRegClass);
...
template <> bool OPEN8DAGToDAGISel::select<ISD::FrameIndex>(SDNode *N) {
  auto DL = CurDAG->getDataLayout();
  ...
  CurDAG->SelectNodeTo(N, OPEN8::FRMIDX,
                       getTargetLowering()->getPointerTy(DL), TFI,
                       CurDAG->getTargetConstant(0, SDLoc(N), MVT::i16));
  return true;
}

template <> bool OPEN8DAGToDAGISel::select<ISD::STORE>(SDNode *N) {
  ...
  unsigned Opc = (VT == MVT::i16) ? OPEN8::STDWSPQRr : OPEN8::STDSPQRr;
...
template <> bool OPEN8DAGToDAGISel::select<ISD::LOAD>(SDNode *N) {
  const LoadSDNode *LD = cast<LoadSDNode>(N);
  ...
  if (unsigned LPMOpc = selectIndexedProgMemLoad(LD, VT)) {
    ...
    switch (VT.SimpleTy) {
    case MVT::i8:
      ResNode = CurDAG->getMachineNode(OPEN8::LPMRdZ, DL, MVT::i8, MVT::Other,
                                       Ptr, RegZ);
      break;
    case MVT::i16:
      ResNode = CurDAG->getMachineNode(OPEN8::LPMWRdZ, DL, MVT::i16,
                                       MVT::Other, Ptr, RegZ);
      ReplaceUses(SDValue(N, 1), SDValue(ResNode, 1));
      break;
    default:
...
template <> bool OPEN8DAGToDAGISel::select<OPEN8ISD::CALL>(SDNode *N) {
  SDValue InFlag;
  ...
  SDNode *ResNode =
    CurDAG->getMachineNode(OPEN8::ICALL, DL, MVT::Other, MVT::Glue, Ops);
...
template <> bool OPEN8DAGToDAGISel::select<ISD::BRIND>(SDNode *N) {
  SDValue Chain = N->getOperand(0);
...
bool OPEN8DAGToDAGISel::selectMultiplication(llvm::SDNode *N) {
  SDLoc DL(N);
  ...
  unsigned MachineOp = isSigned ? OPEN8::MULSRdRr : OPEN8::MULRdRr;
  ...
  if (N->hasAnyUseOfValue(0)) {
    SDValue CopyFromLo =
        CurDAG->getCopyFromReg(InChain, DL, OPEN8::R0, Type, InGlue);
  ...
  if (N->hasAnyUseOfValue(1)) {
    SDValue CopyFromHi =
        CurDAG->getCopyFromReg(InChain, DL, OPEN8::R1, Type, InGlue);
...
void OPEN8DAGToDAGISel::Select(SDNode *N) {
...
bool OPEN8DAGToDAGISel::trySelect(SDNode *N) {
  ...
  case OPEN8ISD::CALL: return select<OPEN8ISD::CALL>(N);
...
FunctionPass *createOPEN8ISelDag(OPEN8TargetMachine &TM,
  ...
  return new OPEN8DAGToDAGISel(TM, OptLevel);
}
