//===-- OPEN8MachineFuctionInfo.h - OPEN8 machine function info -----*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file declares OPEN8-specific per-machine-function information.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_OPEN8_MACHINE_FUNCTION_INFO_H
#define LLVM_OPEN8_MACHINE_FUNCTION_INFO_H

#include "llvm/CodeGen/MachineFunction.h"

namespace llvm {

/// Contains OPEN8-specific information for each MachineFunction.
class OPEN8MachineFunctionInfo : public MachineFunctionInfo {
...
public:
  OPEN8MachineFunctionInfo()
      : HasSpills(false), HasAllocas(false), HasStackArgs(false),
        IsInterruptHandler(false), IsSignalHandler(false),
        CalleeSavedFrameSize(0), VarArgsFrameIndex(0) {}

  explicit OPEN8MachineFunctionInfo(MachineFunction &MF)
      : HasSpills(false), HasAllocas(false), HasStackArgs(false),
        CalleeSavedFrameSize(0), VarArgsFrameIndex(0) {
    unsigned CallConv = MF.getFunction().getCallingConv();

    this->IsInterruptHandler = CallConv == CallingConv::OPEN8_INTR || MF.getFunction().hasFnAttribute("interrupt");
    this->IsSignalHandler = CallConv == CallingConv::OPEN8_SIGNAL || MF.getFunction().hasFnAttribute("signal");
  }
...
#endif // LLVM_OPEN8_MACHINE_FUNCTION_INFO_H
