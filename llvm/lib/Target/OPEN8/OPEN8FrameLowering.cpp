...
MachineBasicBlock::iterator OPEN8FrameLowering::eliminateCallFramePseudoInstr(
  ...
  if (Amount != 0) {
    ...
    if (Opcode == TII.getCallFrameSetupOpcode()) {
      ...
      BuildMI(MBB, MI, DL, TII.get(OPEN8::SPREAD), OPEN8::R3R2).addReg(OPEN8::SP);
      //Amount++;
      Amount = -Amount;
      MachineInstr *New = BuildMI(MBB, MI, DL, TII.get(OPEN8::ADIWRdK), OPEN8::R3R2)
                              .addReg(OPEN8::R3R2, RegState::Kill)
                              .addImm(Amount);
      ...
      // instructions.
      fixStackStores(MBB, MI, TII, OPEN8::R3R2); //true
    } else {
...
void OPEN8FrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  if (hasFP(MF)) {
    SavedRegs.set(OPEN8::R7);
    SavedRegs.set(OPEN8::R6);
  }
}
