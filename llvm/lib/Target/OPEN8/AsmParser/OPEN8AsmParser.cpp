//===---- OPEN8AsmParser.cpp - Parse OPEN8 assembly to MCInst instructions ----===//
//
...
#define DEBUG_TYPE "open8-asm-parser"

using namespace llvm;

namespace {
/// Parses OPEN8 assembly from a stream.
class OPEN8AsmParser : public MCTargetAsmParser {
...
#include "OPEN8GenAsmMatcher.inc"
  ...
  unsigned toDREG(unsigned Reg, unsigned From = OPEN8::sub_lo) {
    MCRegisterClass const *Class = &OPEN8MCRegisterClasses[OPEN8::DREGSRegClassID];
...
public:
  OPEN8AsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
/// An parsed OPEN8 assembly operand.
class OPEN8Operand : public MCParsedAsmOperand {
  typedef MCParsedAsmOperand Base;
  enum KindTy { k_Immediate, k_Register, k_Token, k_Memri } Kind;

public:
  OPEN8Operand(StringRef Tok, SMLoc const &S)
      : Base(), Kind(k_Token), Tok(Tok), Start(S), End(S) {}
  OPEN8Operand(unsigned Reg, SMLoc const &S, SMLoc const &E)
      : Base(), Kind(k_Register), RegImm({Reg, nullptr}), Start(S), End(E) {}
  OPEN8Operand(MCExpr const *Imm, SMLoc const &S, SMLoc const &E)
      : Base(), Kind(k_Immediate), RegImm({0, Imm}), Start(S), End(E) {}
  OPEN8Operand(unsigned Reg, MCExpr const *Imm, SMLoc const &S, SMLoc const &E)
...
public:
  ...
  static std::unique_ptr<OPEN8Operand> CreateToken(StringRef Str, SMLoc S) {
    return std::make_unique<OPEN8Operand>(Str, S);
  }

  static std::unique_ptr<OPEN8Operand> CreateReg(unsigned RegNum, SMLoc S,
                                               SMLoc E) {
    return std::make_unique<OPEN8Operand>(RegNum, S, E);
  }

  static std::unique_ptr<OPEN8Operand> CreateImm(const MCExpr *Val, SMLoc S,
                                               SMLoc E) {
    return std::make_unique<OPEN8Operand>(Val, S, E);
  }

  static std::unique_ptr<OPEN8Operand>
  CreateMemri(unsigned RegNum, const MCExpr *Val, SMLoc S, SMLoc E) {
    return std::make_unique<OPEN8Operand>(RegNum, Val, S, E);
  }
...
bool OPEN8AsmParser::invalidOperand(SMLoc const &Loc,
  ...
  if (ErrorInfo != ~0U) {
    if (ErrorInfo >= Operands.size()) {
      ...
      OPEN8Operand const &Op = (OPEN8Operand const &)*Operands[ErrorInfo];
...
bool OPEN8AsmParser::missingFeature(llvm::SMLoc const &Loc,
                                  uint64_t const &ErrorInfo) {
  return Error(Loc, "instruction requires a CPU feature not currently enabled");
}

bool OPEN8AsmParser::emit(MCInst &Inst, SMLoc const &Loc, MCStreamer &Out) const {
  Inst.setLoc(Loc);
...
bool OPEN8AsmParser::MatchAndEmitInstruction(SMLoc Loc, unsigned &Opcode,
...
int OPEN8AsmParser::parseRegisterName(unsigned (*matchFn)(StringRef)) {
  StringRef Name = Parser.getTok().getString();

  int RegNum = matchFn(Name);

  // GCC supports case insensitive register names. Some of the OPEN8 registers
...
int OPEN8AsmParser::parseRegisterName() {
...
int OPEN8AsmParser::parseRegister(bool RestoreOnFailure) {
  int RegNum = OPEN8::NoRegister;
  ...
  if (Parser.getTok().is(AsmToken::Identifier)) {
    ...
    if (Parser.getLexer().peekTok().is(AsmToken::Colon)) {
      ...
      if (RegNum == OPEN8::NoRegister && RestoreOnFailure) {
...
bool OPEN8AsmParser::tryParseRegisterOperand(OperandVector &Operands) {
  int RegNo = parseRegister();

  if (RegNo == OPEN8::NoRegister)
    return true;

  AsmToken const &T = Parser.getTok();
  Operands.push_back(OPEN8Operand::CreateReg(RegNo, T.getLoc(), T.getEndLoc()));
  Parser.Lex(); // Eat register token.

  return false;
}

bool OPEN8AsmParser::tryParseExpression(OperandVector &Operands) {
  ...
  Operands.push_back(OPEN8Operand::CreateImm(Expression, S, E));
  return false;
}

bool OPEN8AsmParser::tryParseRelocExpression(OperandVector &Operands) {
  ...
  ModifierKind = OPEN8MCExpr::getKindByName(ModifierName.str().c_str());
  ...
  if (ModifierKind != OPEN8MCExpr::VK_OPEN8_None) {
    ...
    if (Parser.getTok().getString() == GENERATE_STUBS &&
      ...
      ModifierKind = OPEN8MCExpr::getKindByName(GSModName.c_str());
      if (ModifierKind != OPEN8MCExpr::VK_OPEN8_None)
  ...
  MCExpr const *Expression = OPEN8MCExpr::create(ModifierKind, InnerExpression,
...
bool OPEN8AsmParser::parseOperand(OperandVector &Operands) {
  ...
  case AsmToken::Minus: {
    ...
    Operands.push_back(OPEN8Operand::CreateToken(Parser.getTok().getString(),
...
OPEN8AsmParser::parseMemriOperand(OperandVector &Operands) {
  ...
  Operands.push_back(OPEN8Operand::CreateMemri(RegNo, Expression, S, E));

  return MatchOperand_Success;
}

bool OPEN8AsmParser::ParseRegister(unsigned &RegNo, SMLoc &StartLoc,
                                 SMLoc &EndLoc) {
  StartLoc = Parser.getTok().getLoc();
  RegNo = parseRegister(/*RestoreOnFailure=*/false);
  EndLoc = Parser.getTok().getLoc();

  return (RegNo == OPEN8::NoRegister);
}

OperandMatchResultTy OPEN8AsmParser::tryParseRegister(unsigned &RegNo,
                                                    SMLoc &StartLoc,
                                                    SMLoc &EndLoc) {
  StartLoc = Parser.getTok().getLoc();
...
bool OPEN8AsmParser::ParseInstruction(ParseInstructionInfo &Info,
                                    StringRef Mnemonic, SMLoc NameLoc,
                                    OperandVector &Operands) {
  Operands.push_back(OPEN8Operand::CreateToken(Mnemonic, NameLoc));
...
bool OPEN8AsmParser::ParseDirective(llvm::AsmToken DirectiveID) {
...
bool OPEN8AsmParser::parseLiteralValues(unsigned SizeInBytes, SMLoc L) {
  MCAsmParser &Parser = getParser();
  OPEN8MCELFStreamer &OPEN8Streamer =
      static_cast<OPEN8MCELFStreamer &>(Parser.getStreamer());
  AsmToken Tokens[2];
  size_t ReadCount = Parser.getLexer().peekTokens(Tokens);
  if (ReadCount == 2 && Parser.getTok().getKind() == AsmToken::Identifier &&
    ...
    OPEN8Streamer.emitValueForModiferKind(Symbol, SizeInBytes, L,
            OPEN8MCExpr::VK_OPEN8_None);
    return false;
  }
  ...
  if (Parser.getTok().getKind() == AsmToken::Identifier &&
    ...
    OPEN8MCExpr::VariantKind ModifierKind =
        OPEN8MCExpr::getKindByName(ModifierName.str().c_str());
    if (ModifierKind != OPEN8MCExpr::VK_OPEN8_None) {
      Parser.Lex();
    ...
    OPEN8Streamer.emitValueForModiferKind(Symbol, SizeInBytes, L, ModifierKind);
    return false;
  }
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeOPEN8AsmParser() {
  RegisterMCAsmParser<OPEN8AsmParser> X(getTheOPEN8Target());
}
...
// Uses enums defined in OPEN8GenAsmMatcher.inc
unsigned OPEN8AsmParser::validateTargetOperandClass(MCParsedAsmOperand &AsmOp,
                                                  unsigned ExpectedKind) {
  OPEN8Operand &Op = static_cast<OPEN8Operand &>(AsmOp);
  ...
  if (Op.isReg()) {
    ...
    if (isSubclass(Expected, MCK_DREGS)) {
      ...
      if (correspondingDREG != OPEN8::NoRegister) {
